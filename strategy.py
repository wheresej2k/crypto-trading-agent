"""Free, rule-based trade signal generator: moving-average crossover, adapted for hourly crypto
bars (the stock bot uses daily bars - see README for why the SMA windows differ here). No API
calls, no cost, fully backtestable. For each symbol: if the short-term average price has moved
above the long-term average, that's bullish (BUY); if it's moved below and we're holding, that's
bearish (SELL, exit); otherwise HOLD.
...
def decide(symbol: str, short_sma: float, long_sma: float, has_position: bool, short_window: int, long_window: int) -> TradeDecision:
    ...
    if spread_pct > 0:
        ...
        reasoning = (
            ...
            f"{long_window}h avg (${long_sma:.4f}) - bullish crossover"
        )
...
def generate_signals(
    ...
    for symbol, bars in bars_by_symbol.items():
        ...
        decisions.append(decide(symbol, short_sma, long_sma, has_position, short_window, long_window))
    return decisions
